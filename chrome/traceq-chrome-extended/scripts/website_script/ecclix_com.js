	...
	chrome.storage.local.get([website_var]).then(async (result) => {
		try{
			...
			if(result[website_var] && result[website_var]['password'] && result[website_var]['username']){
				...
				let username_input = document.getElementById("ctl00_Content_UserName");
				let password_input = document.getElementById("ctl00_Content_Password1");
