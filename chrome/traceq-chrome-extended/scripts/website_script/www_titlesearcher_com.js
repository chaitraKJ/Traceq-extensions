console.log("TraceQ - Title-Searcher script injected");

const website_var = "www_titlesearcher_com";
	...
	chrome.storage.local.get([website_var]).then(async (result) => {
		try{
			...
			if(result[website_var] && result[website_var]['password'] && result[website_var]['username']){
				...
				let username_input = document.querySelector("form input[name='userName']");
				let password_input = document.querySelector("form input[name='password']");
